...
export default class LedSetting extends React.Component{
    ...
    componentDidMount(){
        ...
        .then((response)=>{
            ...
            if (response&&(response.code==0)) {
                self.setState({
                    wifi_led: response.result[3] == 'on' ? true : false,
                    visLoading:false
                });
            }
    ...
    switchValueChange = (value) => {
        ...
        Device.getDeviceWifi().callMethod('set_wifi_led', [value])
        .then((response)=>{
                ...
                <View style={styles.contentView}>
                    <View style={styles.rowContainer}>
                            <View style={[{justifyContent:'flex-start',paddingLeft:10,},styles.cellContentView,]}>
                                <Text style={[styles.title,{paddingLeft:10,}]}>{LocalizedStrings.wifi_led_name}</Text>
                            </View>
